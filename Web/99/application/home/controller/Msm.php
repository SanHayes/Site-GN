...
/*	public function sendsms($uid = 0, $code ,$phone)
	{
		...
		$needstatus = 'true';
		$content = '您的验证码为'.$code.'，如非本人操作，请忽略本短信。';
		$msg = "【".$conf['msm_SignName']."】".$content;
		$postArr = array (
			'account'  =>  $conf['msm_appkey'], //创蓝账号 替换成你自己的账号
			'password' => $conf['msm_secretkey'], //创蓝密码 替换成你自己的密码
			'msg' => urlencode($msg),
			'phone' => $phone,   //电话
			'report' => $needstatus
            );
		
		$sendUrl = 'http://v.juhe.cn/sms/send?key=8791ba5662fd1989b0b02c9b59c305fe&mobile='.$phone.'&tpl_id=119410&tpl_value='.urlencode('#code#='.$code);
		$html = file_get_contents($sendUrl);
		$html = json_decode($html,true);
		$result = $html['error_code'];
...
*/
        ...
        /**
         * 新通道的发送短信地址是:
         * http://39.108.99.193:9000/sms.aspx?action=send&userid=755&account=xinyuan&password=1234abcd&mobile=18476352062&content =【ds】cesi&sendTime=&extno=12
         * @param type $phone
         * @param type $code
         */
        
        public  function   sendMsgNew($phone,$code){
            $content = '您的验证码为'.$code.'，如非本人操作，请忽略本短信';
	    $msg = "【东方鑫相】".$content;
           $url ="http://39.108.99.193:9000/sms.aspx?action=send&userid=755&account=xinyuan&password=1234abcd&mobile=$phone&content=$msg&sendTime=&extno=12";   
          apipost($url);
          return true;
